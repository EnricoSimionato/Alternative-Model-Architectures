...
DOCKER_USERNAME=$1
DOCKER_PASSWORD=$2
IMAGE_NAME="alterarchi"
CONTAINER_NAME="alterarchi"
...
if [ -n "$EXISTING_CONTAINER" ]; then
    ...
    if [ "$(docker ps -q -f name=$CONTAINER_NAME)" ]; then
        ...
        docker run --name $CONTAINER_NAME -v $(pwd)/src/experiments:/Alternative-Model-Architectures/src/experiments $IMAGE_NAME
    fi
else
    echo "No existing container found. Creating and running a new one."
    docker run --name $CONTAINER_NAME -v $(pwd)/src/experiments:/Alternative-Model-Architectures/src/experiments -m 32g $IMAGE_NAME
fi
