...
def perform_global_matrices_initialization_analysis(
    ...
    if file_available:
        ...
        for tensor_wrappers_group_dict in grouped_tensor_wrappers:
            ...
            tensor_wrappers_key_for_analysis = tensor_wrappers_group_dict["label"]
            verbose.print(f"Analyzing the tensors with label {tensor_wrappers_key_for_analysis}")
            ...
            tensors_to_analyze_ab = AnalysisTensorDict(
                [tensor_wrappers_key_for_analysis] * len(tensor_wrappers_group), tensor_wrappers_group
            )
            ...
            tensor_factorizations_dict = {
                ...
                "AB pseudo-inverse initialized": [tensors_to_analyze_pseudo_inverse, pseudo_inverse_init_time],
            ...
            all_groups_tensor_factorizations_loss_histories.append(tensor_factorizations_loss_histories)

        # Saving the loss histories to a file
        with open(file_path, "wb") as f:
            pkl.dump((all_groups_tensor_factorizations_loss_histories, grouped_tensor_wrappers), f)

    for tensor_factorizations_loss_histories in all_groups_tensor_factorizations_loss_histories:
        # Creating the figure to plot the results
        ...
        # Saving the plot
        plt.savefig(os.path.join(directory_path, file_name_no_format + "_plot.png"))
