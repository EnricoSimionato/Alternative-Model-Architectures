...
from .config import KFCLoraConfig
from .gptq import dispatch_gptq
from .layer import Conv2d, KFCLoraLayer, dispatch_default
...
class KFCLoraModel(BaseTuner):
    """
    Creates KFC Low Rank Adapter (LoRA) model from a pretrained transformers model.

    The method is described in detail in https://arxiv.org/abs/2106.09685.

    Args:
        model ([`torch.nn.Module`]): The model to be adapted.
        config ([`KFCLoraConfig`]): The configuration of the Lora model.
    ...
    **Attributes**:
    ...
    prefix: str = "kfc_lora_"

    def __init__(self, model, config, adapter_name) -> None:
        super().__init__(model, config, adapter_name)

    def _check_new_adapter_config(self, config: KFCLoraConfig) -> None:
        """
    ...
    def set_alpha(
        ...
        """
        Set the alpha value of the adapter.
    ...
    def _prepare_model(self, peft_config: KFCLoraConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        if isinstance(target, KFCLoraLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, KFCLoraLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, KFCLoraLayer):
